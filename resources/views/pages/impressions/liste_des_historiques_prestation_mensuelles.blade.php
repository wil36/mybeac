...
<head>
	<meta charset="utf-8" />
	<title>Historique prestation mensuelle</title>
	<style>
		body {
			max-width: 800px;
			margin: 0 auto;
...
<body
  ...
  margin-right: auto;">
	...
	<div id="content">
		...
		<br><br>
		<div class="obj">
			<h1 style="text-align: center;"><b>Historique prestation mensuelle</b></h1>
			<br />
		</div>
					...
					<tbody style="text-align: center;">
						@foreach ($datas as $data)
							<tr>
								<td>{{ $data->annee }}</td>
								<td>
									{{ Carbon\Carbon::create()->day(1)->month($data->mois)->Format('M') }}
								</td>
								<td>{{ number_format($data->montant, 0, ',', ' ') . ' FCFA' }}</td>
								<td>{{ number_format($data->nombre_user, 0, ',', ' ') }}</td>
