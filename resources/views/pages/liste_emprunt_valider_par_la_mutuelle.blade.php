															...
															<table class="nk-tb-list nk-tb-ulist" id="historiqueEmpruntBL" data-auto-responsive="true">
																<thead>
																	<tr class="nk-tb-item nk-tb-head">
																		...
																		<th class="nk-tb-col"><span class="sub-text">@lang('Statut')</span></th>
	...
	<script>
		...
		$(document).ready(function() {
			$('#historiqueEmpruntBL').DataTable({
				...
				ajax: "{{ route('emprunt.getHistoriqueBLAllEmpruntValiderAjax') }}",
				order: [
					[0, "desc"]
				],
				columns: [{
		...
		$('#historiqueEmpruntBLI').DataTable({
			...
			ajax: "{{ route('emprunt.getHistoriqueBLIAllEmpruntValiderAjax') }}",
			order: [
				[0, "desc"]
			],
			columns: [{
		...
		$('#historiqueEmpruntBBL').DataTable({
			...
			ajax: "{{ route('emprunt.getHistoriqueBBLAllEmpruntValiderAjax') }}",
			order: [
				[0, "desc"]
			],
			columns: [{
		...
		$('#historiqueEmpruntASS').DataTable({
			...
			ajax: "{{ route('emprunt.getHistoriqueASSAllEmpruntValiderAjax') }}",
			order: [
				[0, "desc"]
			],
			columns: [{
		...
		$('#historiqueEmpruntASG').DataTable({
			...
			ajax: "{{ route('emprunt.getHistoriqueASGAllEmpruntValiderAjax') }}",
			order: [
				[0, "desc"]
			],
			columns: [{
